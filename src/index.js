...
function isAIInferenceRequest(request, url) {
  ...
  const aiEndpoints = ['/v1/chat/completions', '/v1/completions', '/v1/messages', '/v1/predictions', '/v1/generate', '/v1/embeddings', '/openai/v1/chat/completions'];
...
function validateRequest(request, url, config = CONFIG) {
  ...
  const allowedMethods = (isGit || isDocker || isAI) ? ['GET', 'HEAD', 'POST', 'PUT', 'PATCH'] : config.SECURITY.ALLOWED_METHODS;
  if (!allowedMethods.includes(request.method)) return { valid: false, error: 'Method not allowed', status: 405 };
  if (url.pathname.length > config.SECURITY.MAX_PATH_LENGTH) return { valid: false, error: 'Path too long', status: 414 };
  return { valid: true };
}
...
function createErrorResponse(message, status, includeDetails = false) {
  const errorBody = includeDetails ? JSON.stringify({ error: message, status, timestamp: new Date().toISOString() }) : message;
  return new Response(errorBody, {
    status,
    headers: addSecurityHeaders(new Headers({ 'Content-Type': includeDetails ? 'application/json' : 'text/plain' }))
  });
}
...
async function handleRequest(request, env, ctx) {
  try {
    ...
    if (isDocker && (url.pathname === '/v2/' || url.pathname === '/v2')) {
      const headers = new Headers({ 'Docker-Distribution-Api-Version': 'registry/2.0', 'Content-Type': 'application/json' });
    ...
    if (!validation.valid) return createErrorResponse(validation.error || 'Validation failed', validation.status || 400);
    ...
    platform = sortedPlatforms.find(key => effectivePath.startsWith(`/${key.replace('-', '/')}/`)) || effectivePath.split('/')[1];
    ...
    if (!isGit && !isDocker && !isAI) {
      ...
      if (rangeHeader) {
        const fullContentKey = new Request(targetUrl, {
          ...
          headers: new Headers([...request.headers.entries()].filter(([k]) => !['range'].includes(k.toLowerCase())))
        });
    ...
    if (isGit || isDocker || isAI) {
      ...
      if (isGit && request.method === 'POST' && url.pathname.endsWith('/git-upload-pack') && !requestHeaders.has('Content-Type')) {
        requestHeaders.set('Content-Type', 'application/x-git-upload-pack-request');
      }

      if (isGit && request.method === 'POST' && url.pathname.endsWith('/git-receive-pack') && !requestHeaders.has('Content-Type')) {
    ...
    while (attempts < config.MAX_RETRIES) {
      try {
        ...
        const finalFetchOptions = isGit || isDocker ? { ...fetchOptions, signal: controller.signal } : { ...fetchOptions, signal: controller.signal };

        if (request.method === 'HEAD') {
          response = await fetch(targetUrl, finalFetchOptions);
          if (response.ok && !response.headers.get('Content-Length')) {
            const getResponse = await fetch(targetUrl, { ...finalFetchOptions, method: 'GET' });
            if (getResponse.ok) {
              const headHeaders = new Headers(response.headers);
              const contentLength = getResponse.headers.get('Content-Length');
              if (contentLength) headHeaders.set('Content-Length', contentLength);
              else {
                const arrayBuffer = await getResponse.arrayBuffer();
                headHeaders.set('Content-Length', arrayBuffer.byteLength.toString());
              }
              response = new Response(null, { status: getResponse.status, statusText: getResponse.statusText, headers: headHeaders });
            }
          }
        } else {
          response = await fetch(targetUrl, finalFetchOptions);
              ...
              if (targetPathParts.length >= 2 && targetPathParts[0] === 'v2') {
                if (targetPathParts[1] === 'library') {
                  ...
                  if (targetPathParts.length >= 3) {
                    ...
                    const repo = repoWithTag.split(':')[0];
                    scope = `repository:${user}/${repo}:pull`;
                  }
                } else {
                  if (targetPathParts.length >= 2) {
                    const repoName = targetPathParts[1].split(':')[0];
                    scope = `repository:library/${repoName}:pull`;
                  }
                }
              }
        ...
        if (attempts >= config.MAX_RETRIES) {
          ...
          return createErrorResponse(`Failed after ${config.MAX_RETRIES} attempts: ${message}`, 500, true);
        }
        await new Promise(resolve => setTimeout(resolve, config.RETRY_DELAY_MS * attempts));
      }
    }
    ...
    if (!response.ok && response.status !== 206) {
      if (isDocker && response.status === 401) {
        ...
        return createErrorResponse(`Authentication required: ${errorText}`, 401, true);
      }
      const errorText = await response.text().catch(() => 'Unknown error');
      return createErrorResponse(`Upstream error (${response.status}): ${errorText}`, response.status, true);
    }
    ...
    if (platform === 'pypi' && response.headers.get('content-type')?.includes('text/html')) {
      ...
      const rewrittenText = originalText.replace(/https:\/\/files\.pythonhosted\.org/g, `${url.origin}/pypi/files`);
    ...
    if (platform === 'npm' && response.headers.get('content-type')?.includes('application/json')) {
      const originalText = await response.text();
      const rewrittenText = originalText.replace(/https:\/\/registry\.npmjs\.org\/([^\/]+)/g, `${url.origin}/npm/$1`);
      responseBody = new ReadableStream({
        start(controller) {
          controller.enqueue(new TextEncoder().encode(rewrittenText));
          controller.close();
    ...
    const finalResponse = new Response(responseBody, { status: response.status, headers });
